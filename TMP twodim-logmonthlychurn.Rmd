...
# some configuration
SLIMDATA <- T
slimfactor <- 0.01 # use this to slim data for faster experimentation
...
if (!exists('customersorg'))
{
  ...
  customersorg <- read.table(file = 'data/CustomersV3.export.tsv', sep = '\t', header = TRUE, stringsAsFactors = F)
  gdp <- read.table(file = 'data/gdp-simple.export.tsv', sep = '\t', header = TRUE, stringsAsFactors=F)
  colnames(gdp)<-c('marketname', 'gdppercapita')
  subsorg <- read.table(file = 'data/SubscriptionsV3.export.tsv', sep = '\t', header = TRUE,
                     stringsAsFactors=F)
}
```
...
```{r}
# find subses that are in a "next should be three-to-one-state"
subs5 <- subs4 %>%
  ...
  dplyr::mutate(isthreetoonestate = ifelse(is.na(isthreetoonestate), 
                       ifelse(!is.na(threetoonestartdate) & threetoonestartdate < periodend + days(-10), 1, 0),
                       isthreetoonestate)) %>%
  dplyr::ungroup() %>%
  dplyr::select(-periodend)
```


```{r}
interestingcusts <- subs5 %>%
  dplyr::filter(months == 1 & isthreetoonesubs == 1) %>%
  dplyr::group_by(customerid) %>%
  dplyr::summarise()


tmp <- subs5 %>%
  dplyr::inner_join(interestingcusts, by = "customerid") %>%
  dplyr::select(customerid, subscriptionid, startmonth, endmonth, months, threetoonestartdate, isthreetoonesubs, isthreetoonestate, revenuecurrinclvat)
